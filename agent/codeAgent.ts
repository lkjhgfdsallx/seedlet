 ...
 *   2. 优先通过工具获取上下文后再修改代码
 *   3. 在多步骤任务中维护待办事项状态
 *   4. 完成后使用 attempt_completion 进行确定性收尾
 *
...
// ---------------------------------------------------------------------------
// 1. Memory 结构 —— 承载 code agent 在步骤间传递的可序列化状态
// ---------------------------------------------------------------------------

/** 代码模式 agent 的功能内存 */
type CodeMemory = ChatMemory & {
  /** 当前阶段：analyzing → implementing → validating → done */
  phase?: "analyzing" | "implementing" | "validating" | "done";
  /** 是否已完成实现 */
...
// ---------------------------------------------------------------------------
// 2. 占位内部工具 —— code 模式下可用的能力，只留空实现 + 注释
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 3. 系统提示 —— code 模式的核心行为约束
// ---------------------------------------------------------------------------

const CODE_SYSTEM_PROMPT = `你是一位技艺精湛的软件工程师 ll，拥有众多编程语言、框架、设计模式和最佳实践方面的深厚知识。
...
- 你可以使用一些工具，这些工具允许你在用户计算机上执行 CLI 命令、列出文件、查看源代码定义、进行正则搜索、读写文件以及提出后续问题。这些工具能帮助你高效完成各种任务，如编写代码、对现有文件进行编辑或改进、了解项目的当前状态、执行系统操作等等。
- 当用户最初向你提出任务时，当前工作区目录（'{{WORKSPACE_PATH}}'）中所有文件路径的递归列表将包含在 environment_details 中。这提供了项目文件结构的概览，通过目录/文件名以及文件扩展名提供关键的项目洞察。
- 当你认为有助于完成用户任务时，可以使用 execute_command 工具在用户计算机上运行命令。当你需要执行 CLI 命令时，必须清晰解释该命令的作用。
...
- 以下是当前可用的模式：
  * "🏗️ 架构师" 模式 (architect) - 当你在实施前需要规划、设计或制定策略时使用此模式。
  * "💻 代码" 模式 (code) - 当你需要编写、修改或重构代码时使用此模式。
  * "🧪 Jest 测试工程师" 模式 (jest-test-engineer) - 当你需要编写、维护或改进 Jest 测试时使用此模式。
...
- 使用 execute_command 工具之前，你必须首先考虑提供的系统信息上下文，以了解用户的环境，并定制命令以确保其与用户系统兼容。
- 某些模式对可以编辑的文件有限制。如果你尝试编辑受限制的文件，操作将被拒绝，并显示 FileRestrictionError，指明当前模式允许的文件匹配模式。
- 在确定合适的结构和需要包含的文件时，务必考虑项目类型，同时考虑哪些文件可能与完成任务最相关。
...
- 你只允许使用 ask_followup_question 工具向用户提问。仅当需要额外细节才能完成任务时使用此工具，并确保使用清晰简洁的问题来帮助你推进任务。
...
语言偏好：
除非用户另有指示，你应始终使用简体中文（zh-CN）语言进行思考和表达。`;
...
/**
 * 严格的终止判断：
 *   1. 优先检查 memory 中 implementationFinished 与 completionRequested
 *   2. 兜底检查最后一条助手消息是否包含明确的完成信号
 */
const isCodeDone = async ({
  messages,
  memory
}: {
  ...
  if (
    memory &&
    memory.implementationFinished === true &&
...
export const agentConfig: AgentConfig<CodeMemory> = {
  name: "code",
  description: "代码模式——编写、修改、重构代码，支持文件操作、搜索和命令执行",
  ...
  controller: async (state) => ({
    ...
    messages: [
      ...state.messages,
      SystemMessage(
        "你似乎陷入了停滞。请回顾当前任务进度，优先读取或搜索相关文件，然后在必要时修改代码，并在完成后使用 attempt_completion 收尾。"
      )
    ]
  })
};
